...
export function findLastMarkerComment(comments: IssueComment[], marker: string): IssueComment | null {
...
export function hasUserReplySince(comments: IssueComment[], marker: string): boolean {
  ...
  return comments.some(
    comment => !comment.body.includes(AGENT_RUNNER_MARKER) && Date.parse(comment.createdAt) > anchorTime
  );
}
